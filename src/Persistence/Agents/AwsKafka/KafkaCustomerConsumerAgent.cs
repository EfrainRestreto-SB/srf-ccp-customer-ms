...
public class KafkaCustomerConsumerAgent<TKey, TValue>(
    IKafkaConsumerConfig kafkaConsumerConfig,
    AWSOptions awsOptions,
    ILogger<KafkaCustomerConsumerAgent<TKey, TValue>> logger
) : IKafkaConsumerAgent<TKey, TValue>, IEquatable<KafkaCustomerConsumerAgent<TKey, TValue>?>
{
    ...
    private async void OauthCallback(IClient client, string cfg)
    {
        try
        {
            ...
            var stsClient = new AmazonSecurityTokenServiceClient(
                ...
                new AmazonSecurityTokenServiceConfig { RegionEndpoint = awsOptions.Region }
            );

            var tokenGenerator = new AWSMSKAuthTokenGenerator(stsClient);

            (string, long) values = await tokenGenerator.GenerateAuthTokenFromCredentialsProvider(
    ...
    public async Task ConsumeMessages(ushort consumerNumber, Func<TKey, TValue, Task> method, CancellationToken cancellationToken)
    {
        ...
        try
        {
            using IConsumer<TKey, TValue> consumer = new ConsumerBuilder<TKey, TValue>(kafkaConsumerConfig.GetConsumerConfig())
                .SetValueDeserializer(new JsonKafkaDeserializer<TValue>()).SetOAuthBearerTokenRefreshHandler(OauthCallback).Build();

            consumer.Subscribe(kafkaConsumerConfig.GetTopicName());

            logger.LogInformation(
                "Consumer inicializado: {ConsumerName} - Thread #{Number} - Topic: {Topic}",
                consumer.Name, consumerNumber, kafkaConsumerConfig.GetTopicName()
            );
        ...
        {
            logger.LogError(ex, "Error al conectarse al Kafka: {Message} {InnerException} {StackTrace}",
    ...
    private async Task ConsumeLoop(IConsumer<TKey, TValue> consumer, Func<TKey, TValue, Task> method, CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                ...
                {
                    object value = await method(cr.Message.Key, cr.Message.Value);
                    consumer.Commit(cr);
            ...
            {
                logger.LogError(e, "Error: Razón {Reason}", e.Error.Reason);
                await Task.Delay(TimeSpan.FromSeconds(2), cancellationToken);
            }
            ...
            {
                logger.LogError(e, "Timeout ocurrido mientras se consumían mensajes");
                await Task.Delay(TimeSpan.FromSeconds(2), cancellationToken);
            }
            ...
            {
                logger.LogError(e, "Error inesperado: {Message}. {InnerException} {StackTrace}", e.Message, e.InnerException, e.StackTrace);
                await Task.Delay(TimeSpan.FromSeconds(2), cancellationToken);
            }
        }
    }
    ...
    {
        return Equals(obj as KafkaCustomerConsumerAgent<TKey, TValue>);
    }

    public bool Equals(KafkaCustomerConsumerAgent<TKey, TValue>? other)
    {
        return other is not null &&
               EqualityComparer<IKafkaConsumerConfig>.Default.Equals(kafkaConsumerConfig, other.kafkaConsumerConfig) &&
               EqualityComparer<AWSOptions>.Default.Equals(awsOptions, other.awsOptions) &&
               EqualityComparer<ILogger<KafkaCustomerConsumerAgent<TKey, TValue>>>.Default.Equals(logger, other.logger);
    }

    public static bool operator ==(KafkaCustomerConsumerAgent<TKey, TValue>? left, KafkaCustomerConsumerAgent<TKey, TValue>? right)
    {
        return EqualityComparer<KafkaCustomerConsumerAgent<TKey, TValue>>.Default.Equals(left, right);
    }
